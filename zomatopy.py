...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", price_min="", price_max="",limit=5):
        ...
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines)+"&average_cost_for_two>=" + str(price_min) + "&average_cost_for_two<=" + str(price_max), headers=headers).content).decode("utf-8")
